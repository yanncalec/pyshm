...
class MxDeconv:
    ...
    def fit(self, constflag=False, tidx0=None, Ntrn=None, method='ls'):
        ...
        self._As = [np.hsplit(self._fit_result[0][n], lag) for n,lag in enumerate(self.lag)]
        return self._fit_result

    def predict(self, X, *args, constflag=True):
        """Prediction (wrapper of the static method _predict()).
...
class DmxDeconv(MxDeconv):
    ...
    # Ws = None  # decorrelated inputs, not useful since it is stored in Xvar
...
class ShfDeconv(shfinvsp, DmxDeconv):
    ...
    _povec = None  # pseudo-inverse of the constant vector 1
    ...
    def __init__(self, Psi, Y, X, lag, *args, **kwargs):
        ...
        self._povec = self.pinv_analysis(np.ones((1, self.dim[0])), axis=-1)
...
class DiffDeconv(ShfDeconv):
    ...
    The differential kernel psi is the auto-convolution of [1,-1]: at degree 0 it is [1,-1], at degree 1 it is [1, -2, 1] etc. The list of kernels Psi here is [psi], and Ker(Psi), or the kernel space generated by Psi, is just the linear space of polynomials of maximum degree equals to that of psi. We use the numpy routine for fast computation of the projection operators.
    """
...
class DiffDeconvBM(DiffDeconv):
    ...
    def fit(self, sigmaq2, sigmar2, x0=0., p0=1., constflag=False):
