...
##### Feature extraction #####
def feature_extraction(X0, scaling, nfreq=None, nbins=None, absflag=True, dflag=True, ddflag=True, cdim=1, vthresh=None):
    """
    Args:
        X0 (2d array): input, each column is a full observation
    ...
    bins = Tools.logscale_bin(nfreq, scaling=scaling)[:nbins]  #
    func = lambda x:Tools.triangle_windowing(x, bins)

    M = []
    for n, x in enumerate(X0):
        c = x[1:, :] if n==0 else x  # drop the DC component which is the first row of X0[0]
        if absflag:
