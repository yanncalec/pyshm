...
def dgp_cov_matrix(Nt, snr2=100, clen2=20):
    ...
    a = 1/clen2
    f = lambda x:(-2*a + 4*(a*x)**2)*np.exp(-a*x**2) # second derivative of f(x)=exp(-a*x**2)
...
def dim_reduction(func):
    ...
    def newfunc(Y0, X0, W0, *args, vthresh=1e-3, corrflag=False, **kwargs):
        ...
        """
        if vthresh > 0:  # dimension reduction
...
#### Interfaces ####
def mts_diffdeconv(Y0, X0, dord=1, lag=1, snr2=None, clen2=None, sidx=0, Ntrn=None, vthresh=0., corrflag=False, pthresh=0., Nexp=0, method="mean"):
    ...
    # prepare regressor
    # regressor = ransac(dim_reduction(data_subset(multi_linear_regression)))
    regressor = random_subset(dim_reduction(percentile_subset(multi_linear_regression)))

    # training data
    (tidx0, tidx1), Ntrn = training_period(X0.shape[1], tidx0=sidx, Ntrn=Ntrn)  # valid training period
    Ytrn, Xtrn = Yvar[:,tidx0:tidx1], Xvar[:,tidx0:tidx1]
    # GLS matrix
    if snr2 is not None and clen2 is not None and dord > 0:
        ...
        _, W = dgp_cov_matrix(Ntrn, snr2=snr2, clen2=clen2)
    else:
        W = None # np.eye(Ntrn)

    # estimation of kernel
    Amat, Cvec, *_ = regressor(Ytrn, Xtrn, W, pthresh=pthresh, vthresh=vthresh, corrflag=corrflag, Nexp=Nexp, method=method)
...
def mts_mutdecorr(Y0, lag=1, vthresh=1e-3, corrflag=False): #, sidx=0, Ntrn=None):
    """Dimension-wise mutual decorrelation of a mts."""
    Yprd = []
    for n in range(Y0.shape[0]):
        ...
        toto, *_ = mts_diffdeconv(Y0[[n],:], Y0[locs,:], lag=lag, vthresh=vthresh, corrflag=corrflag) #, sidx=sidx, Ntrn=Ntrn)
...
def mts_ssproj(X0, cdim=None, vthresh=1e-1, corrflag=False):
