...
def Download_data(dbdir, options):
    ...
    # get the token
    payload = {"token":"null","action":"loginClient","data":{"email":options.login,"password":options.password}}
    ...
    for pid in ListPID:  # iteration on the PID
        projdir = os.path.join(dbdir, str(pid)) # output directory
    ...
    if options.delete:
        ...
        try:
            for pid in failedPID_list:
                projdir = os.path.join(dbdir, str(pid))
...
def main():
    ...
    parser.add_argument('--login', dest='login', type=str, default='be@osmos-group.com', help="login of Osmos server (default='be@osmos-group.com')", metavar='string')
    parser.add_argument('--pw', dest='password', type=str, default='osmos', help="password of Osmos server (default='osmos')", metavar='string')
